...
func main() {
    //First we will look at encoding basic data types to JSON strings. Here are some examples for atomic values.
    ...
    //Now let us look at decoding JSON data into Go values. Her is an example for a generic data structure.
    ...
    fmt.Println(dat)
    //In order to use the values in the decoded map, we will need to cast them to their appropriate type. For example here we cast the value in num to the expected float64 type.
    num := dat["num"].(float64)
    fmt.Println(num)
